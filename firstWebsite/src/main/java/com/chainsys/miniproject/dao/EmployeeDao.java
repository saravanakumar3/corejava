...
//		int rows ;
		...
		try {
			...
			ps.setString(1, newemp.getEmail());
...
// for updating all the columns of the table
	public static int updateEmployee(Employee newemp) {
		...
		try {
			...
			ps.setString(7, newemp.getEmail());
		...
		try {
			...
			emp = new Employee();
			   if(rs.next()) {
		          ...
		          java.util.Date date = new java.util.Date(rs.getDate(5).getTime());//why getTime method used ok now clear 
